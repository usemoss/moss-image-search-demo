...
const CORS_ORIGINS = (process.env.MOSS_CORS_ORIGINS ?? "http://localhost:5173,http://localhost:4173").split(",");
...
function getIndexName(tier) {
    return `${BASE_INDEX_NAME}-${tier}`;
}
async function ensureIndexLoaded(indexName) {
...
async function isPrivateOrLoopback(hostname) {
